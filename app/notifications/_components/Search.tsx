...
export default function Search() {
  ...
  const handleRemove = (id: number) => {
    ...
    const updatedStorage = existing.filter(
      (storedId: string) => Number(storedId) !== id
    );
      ...
      <section className="mt-5">
        ...
        <ul className="flex flex-wrap gap-y-2 gap-x-2 outline-1 outline-muted p-2 rounded-xl">
          {selectedProducts.length === 0 && (
            <p>Nothing selected, add products below!</p>
          )}
          {selectedProducts.map((product) => (
            <li
              key={product.id}
              className="bg-gray-200 rounded-xl pl-2 p-1 flex gap-x-2"
            >
              {product.name}
              <button
                className="!outline-0"
